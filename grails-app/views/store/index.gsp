		...
		<div id="list-store" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<g:sortableColumn property="cnpj" title="${message(code: 'store.cnpj.label', default: 'Cnpj')}" />
					
						<g:sortableColumn property="password" title="${message(code: 'store.password.label', default: 'Password')}" />
					
						<g:sortableColumn property="razaoSocial" title="${message(code: 'store.razaoSocial.label', default: 'Razao Social')}" />
				...
				<g:each in="${storeInstanceList}" status="i" var="storeInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${storeInstance.id}">${fieldValue(bean: storeInstance, field: "cnpj")}</g:link></td>
					
						<td>${fieldValue(bean: storeInstance, field: "password")}</td>
					
						<td>${fieldValue(bean: storeInstance, field: "razaoSocial")}</td>
